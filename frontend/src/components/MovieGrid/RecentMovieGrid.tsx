...
import { fetchRecentMovies } from "../../pages/RecentMovies/RecentMovieLogic";
import BookingPopup from "../BookingPopup/BookingPopup.tsx";
import styles from "./MovieGrid.module.css";

interface RecentMovieGridProps {
  title?: string;
  limit?: number;
  showTitle?: boolean;
  gridColumns?: number;
...
export default function RecentMovieGrid({
  title = "Phim Đang Chiếu",
  limit = 8,
  showTitle = true,
  gridColumns = 4,
}: RecentMovieGridProps) {
  ...
  useEffect(() => {
    const loadMovies = async () => {
      try {
        ...
        setError("Failed to load recent movies. Please try again.");
      } finally {
        setLoading(false);
      }
    };
